...
@userge.cmd("gtofertas", about={"header": "GTOfertas sender"}, allow_via_bot=False)
    ...
    for message_count in range(int(max_messages)):
        try:
            if message_count >= int(max_messages):
                break
            i = await userge.forward_messages(chat_id=-1001115033767, from_chat_id=-1001197236241, messages_ids=message_reply)
            message_reply += 1
            if i:
